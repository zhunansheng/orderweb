        ...
        <div class="edittable-testauto-con">
          <Form :model="dictionary" :label-width="80" ref="generation">
            ...
            <FormItem label="数据库名称:">
              <Checkbox :indeterminate="dictionary.indeterminate" :value="dictionary.checkAll" @click.prevent.native="dicCheckAll">全选</Checkbox>
...
export default {
  ...
  methods: {
    ...
    testlink () {
      axios.put(util.url + '/management_sql/', {
          'ip': this.formItem.ip,
    ...
    add () {
      ...
      this.$refs['formValidate'].validate((valid) => {
        if (valid) {
          ...
          axios.post(util.url + '/management_sql/', {
    ...
    delbaselink () {
      if (this.delbasename === this.delconfirmbasename) {
        axios.delete(`${util.url}/management_sql?del=${this.delbasename}`)
          .then(res => {
            this.$Notice.success({
              title: '通知',
              desc: res.data
    ...
    mountdata (vl = 1) {
      axios.get(`${util.url}/management_sql?page=${vl}&permissions_type=base`)
        .then(res => {
