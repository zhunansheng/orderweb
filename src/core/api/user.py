...
class gitinfoview(baseview.SuperUserpermissions):
    '''
    ...
    def get(self, request, args=None):
        if args == 'all':
            try:
                ...
                try:
                    page_number = gitinfo.objects.aggregate(alter_number=Count('id'))
                    start = int(page) * 10 - 10
                    end = int(page) * 10
                    info = gitinfo.objects.all()[start:end]
                    serializers = gitinfoser(info, many=True)
    ...
    def post(self, request, args=None):
        try:
            ...
            project_item = request.data['project_item']
            progress_name = request.data['progress_name']
            gitaddress = request.data['gitaddress']
            hostname = request.data['hostname']
            newgit = gitinfo.objects.create(
                project=project,
            ...
            return Response('%s 保存成功!' % progress_name)
...
class ldapauth(baseview.AnyLogin):
    ...
    def post(self, request, args: str = None):
        try:
            ...
            jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
            jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER
            valite = Ldap().validate(user, password)
            print(valite)
            if valite:
                try:
