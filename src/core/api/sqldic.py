...
def downloaderrorLog(req):
    '''
        导出工单关联的错误信息
    '''
    workid = req.GET['workid']
    data_tmp = SqlRecord.objects.filter(workid=workid, state__icontains='failed').values('state')
    return JsonResponse(list(data_tmp), safe=False)